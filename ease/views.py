...
def index(request):
    ...
    completed = FMModel.objects.filter(completed=True).count() + TVModel.objects.filter(completed=True).count()
    pending = FMModel.objects.filter(completed=False).count() + TVModel.objects.filter(completed=False).count()
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        username = unames.gen_username(first_name_list[0] + "."+ last_name_list[0],)
        ...
        user = User.objects.create_user(first_name=first_name, last_name=last_name, email=email, password=password, username=username.lower())
        # save user
        user.save()
        ...
        nsp.save()
        print(nsp)
        # send user an sms with login details
        # send user an email with login details
...
def fm_general(request):
    return render(request, 'ease/fm_general.html')

@login_required(login_url=LOGIN_URL, redirect_field_name=REDIRECT_FIELD_NAME)
def fm_single(request,id):
    return render(request, 'ease/fm_single.html')
